...
export function RuntimeChart({ result, workflow, onSelect }: { result:SimulationResult; workflow:Workflow; onSelect:(event:SimulationEvent)=>void }) {
  ...
  const preferred=workflow.name.name==="device_alert"?monitored:["oxygen","heart_rate",...monitored];
  ...
  return <section className="panel overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#dce5e2] px-4 py-3"><div><b className="text-sm">Runtime signal analysis</b><p className="mt-0.5 text-[11px] text-[#71807c]">Synthetic input series from simulation steps · markers from runtime events</p></div><div className="flex flex-wrap gap-2">{Object.entries(markerMeta).filter(([kind])=>markers.some((marker)=>marker.kind===kind)).map(([kind,meta])=><span key={kind} className="flex items-center gap-1.5 text-[10px] font-semibold text-[#667672]"><span className="h-2 w-2 rounded-full" style={{background:meta.color}}/>{meta.short}</span>)}</div></div><div className="bg-[#fbfcfc] p-3 lg:p-5">
